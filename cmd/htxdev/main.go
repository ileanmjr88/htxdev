...
// the site reads. serve hands the same JSON to anybody who asks over HTTP.
package main
...
usage:
  ...
  htxdev serve [flags]    serve the events over HTTP
  htxdev help             this message

run "htxdev <command> -h" for a command's flags.
`)
}
